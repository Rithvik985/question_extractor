...
# Main execution function for current directory
def convert_all_docx_in_current_dir():
    """
    Convert all DOCX files in current directory to PDF in the same directory
    """
    current_dir = os.getcwd()
    docx_files = list(Path(current_dir).glob('*.docx'))
    
    if not docx_files:
        print("No DOCX files found in current directory.")
        return
    
    print(f"Found {len(docx_files)} DOCX file(s) in current directory:")
    ...
    for docx_file in docx_files:
        ...
        result = convert_docx_to_pdf_libreoffice(docx_path, current_dir)
        if result:
            successful_conversions += 1
            success = True
            print(f"  ✅ Success: {docx_file.name} → {Path(pdf_path).name}")
    ...
    if successful_conversions > 0:
        print(f"\n✅ {successful_conversions} PDF file(s) created in current directory!")

def main():
    """
    Main function - converts all DOCX files in current directory
    """
    print("DOCX to PDF Batch Converter")
    print("Converting all DOCX files in current directory...")
    print("=" * 50)
    
    convert_all_docx_in_current_dir()
